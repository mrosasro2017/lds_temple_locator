...
let state=$("#select_state").find('option:selected').val();
...
function showTemples () {
	...
	state=$("#select_state").find('option:selected').val();

	console.log(state);

	temples = new ol.layer.Image({
		source: new ol.source.ImageWMS({
			url: 'https://tethys.byu.edu/geoserver/LDS_Temples_USA/wms',
			params: { 'LAYERS': state },
	...
	ajax_url = 'https://tethys.byu.edu/geoserver/LDS_Temples_USA/wfs?request=GetCapabilities';
		...
		if (wms_url) {
			...
			$.ajax({
				...
				success: function (result) {
					...
					templestate = result["features"][0]["properties"]["State"];
					...
					$("#temple-info").append('<h3 id="Temple-Name-Tab">Temple: '+ templename
						...
						+ templecity + '</h5><h5 id="Temple-State-Tab">State: '
						+ templestate + '</h5><h5 id="Temple-Telephone-Tab">Phone: '
